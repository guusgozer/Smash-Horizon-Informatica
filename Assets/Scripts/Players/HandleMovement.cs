...
public class HandleMovement : MonoBehaviour {
    ...
    void HorizontalMovement()
    {
        ...
        {
            rb.AddForce(new Vector2((states.horizontal * actualSpeed) - rb.linearVelocity.x * this.acceleration, 0));
        }
        ...
        if (states.vertical > 0)
        {
            if (!justJumped)
            {
                ...
                if (states.onGround)
                {
                    ...
                    rb.linearVelocity = new Vector3(rb.linearVelocity.x, this.jumpSpeed);
                    jmpTimer = 0;
